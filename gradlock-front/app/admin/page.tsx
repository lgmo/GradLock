...
export default function AdminPage() {
  ...
  const loadRooms = async () => {
    try {
      ...
      setError('Erro ao carregar salas. Verifique se o servidor está funcionando.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveRoom = async (roomData: CreateRoomData | UpdateRoomData) => {
    try {
      if (modalMode === 'create') {
        const newRoom = await RoomService.createRoom(roomData as CreateRoomData);
        setRooms([...rooms, newRoom]);
        showSnackbar('Sala criada com sucesso!', 'success');
      } else if (selectedRoom) {
        const updatedRoom = await RoomService.updateRoom(selectedRoom.id, roomData as UpdateRoomData);
    ...
    <>
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={4}>
          <Typography variant="h4" component="h1" fontWeight="bold" sx={{ color: '#4C56F8' }}>
            Gerenciar Salas
          </Typography>
          <Button 
            variant="contained" 
